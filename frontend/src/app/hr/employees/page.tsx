...
const StatusBadge = ({ status }: { status: string }) => {
  const styles: Record<string, string> = {
    ...
    DOCUMENTS_UPLOADED: 'bg-purple-500/10 text-purple-600 border-purple-500/20',
...
const ProfileBar = ({ value }: { value: number }) => (
  <div className="flex items-center gap-2">
    ...
    <span className="text-[10px] text-muted-foreground font-mono w-8">{value}%</span>
  </div>
);
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-secondary border border-border rounded-xl pl-10 pr-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-blue-500 transition-colors"
              />
            </div>
            ...
            <select
              ...
              className="bg-secondary border border-border rounded-xl px-4 py-2.5 text-sm text-foreground focus:outline-none focus:border-blue-500 transition-colors"
            >
                    ...
                    <tr key={emp.id} className="hover:bg-secondary/30 transition-colors text-sm">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center font-heading text-xs font-bold text-foreground uppercase shrink-0">
                      ...
                      {/* Contact */}
                      <td className="px-5 py-4">
                        <div className="text-card-foreground text-xs">{emp.user?.email}</div>
                        <div className="text-muted-foreground text-[10px] mt-0.5">{emp.phone || 'N/A'}</div>
                      </td>

                      {/* Department + Designation */}
                      <td className="px-5 py-4">
                        <div className="text-card-foreground text-xs font-medium">{emp.department?.name || '—'}</div>
                        <div className="text-muted-foreground text-[10px] mt-0.5">{emp.designation?.name || '—'}</div>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex flex-col gap-1">
                          ...
                          {!emp.user?.isActive && (
                            <span className="text-[9px] text-red-600 font-bold uppercase">Deactivated</span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex justify-end items-center gap-1">
                          ...
                          <button
                            ...
                            className="p-1.5 hover:bg-secondary rounded-lg text-muted-foreground hover:text-emerald-600 transition-colors"
                            title="Edit employee"
                          >
                          ...
                          <Link
                            ...
                            className="p-1.5 hover:bg-amber-500/10 rounded-lg text-muted-foreground hover:text-amber-600 transition-colors"
                          ...
                          <button
                            ...
                            className="p-1.5 hover:bg-secondary rounded-lg text-muted-foreground hover:text-purple-600 transition-colors"
                  ...
                  <tr>
                    <td colSpan={7} className="px-5 py-16 text-center">
                      <div className="flex flex-col items-center gap-3">
                        <Users className="w-12 h-12 text-card-foreground" />
                        <span className="text-muted-foreground font-medium">No employees found matching the current filters.</span>
                        <button
                          onClick={() => setShowCreate(true)}
                          className="text-blue-600 hover:text-blue-300 text-sm font-semibold flex items-center gap-1"
                        >
            ...
            <div className="flex items-center justify-between border-t border-border px-5 py-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={page === 1}
                  className="p-2 rounded-lg bg-secondary hover:bg-secondary disabled:opacity-30 text-muted-foreground hover:text-foreground transition-colors border border-border"
                >
