    ...
    <HRLayout>
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Open Tickets" value={stats?.open} icon={MessageSquare} color="bg-blue-500/10 text-blue-600" subtitle={`${stats?.highPriority || 0} High Priority`} />
          ...
          <StatCard title="Resolved Cases" value={stats?.resolved} icon={CheckCircle2} color="bg-emerald-500/10 text-emerald-600" subtitle="Ready for closure" />
          <StatCard
        ...
        <div className="bg-secondary border border-border rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            ...
            <select
              ...
              className="bg-background border border-border rounded-xl px-4 py-2.5 text-xs text-foreground focus:outline-none focus:border-blue-500 appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%23666%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E')] bg-[length:0.65em_auto] bg-[right_1rem_center] bg-no-repeat"
            >
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-secondary">
                  <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-6 py-4">Ticket</th>
                  <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-6 py-4">Raised By</th>
                  <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-6 py-4">Department</th>
                  ...
                  <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-6 py-4">Assignee</th>
              ...
              <tbody className="divide-y divide-neutral-800/40">
                {isLoading ? (
                  <tr>
                    <td colSpan={8} className="text-center py-20 text-muted-foreground text-xs font-semibold">
                  ...
                  <tr>
                    <td colSpan={8} className="text-center py-20 text-muted-foreground text-xs">
                      <div className="flex flex-col items-center gap-3">
                        <div className="w-12 h-12 bg-secondary rounded-2xl flex items-center justify-center">
                          <LifeBuoy className="w-6 h-6 text-muted-foreground" />
                        </div>
                        <p className="text-muted-foreground">No support tickets match the selected filters.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-secondary/50 transition-colors">
                      <td className="px-6 py-4">
                        <div>
                          ...
                          <p className="text-[9px] font-mono text-muted-foreground font-bold mt-0.5">{item.complaintNumber}</p>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-xs font-semibold text-foreground">
                        <div className="flex flex-col gap-0.5">
                          <div className="flex items-center gap-2">
                            <span className={item.raisedByName === 'Unknown Employee' ? 'text-muted-foreground italic' : ''}>
                      ...
                      <td className="px-6 py-4 text-xs text-card-foreground">
                        {item.assignedTo ? `${item.assignedTo.firstName} ${item.assignedTo.lastName}` : <span className="text-muted-foreground italic">Unassigned</span>}
                      </td>
