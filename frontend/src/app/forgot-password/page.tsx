...
export default function ForgotPasswordPage() {
  ...
  const forgotMutation = useMutation({
    ...
    onSuccess: (data) => {
      setSuccess(data.message || 'Reset instructions have been logged to the server terminal. In production, an email is sent.');
      setError('');
    },
    onError: (err: any) => {
          ...
          {success ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <p className="text-xs text-neutral-400 leading-relaxed">
                Enter your registered corporate email address and we will generate a password recovery token.
              </p>
              ...
              >
                ...
                {forgotMutation.isPending ? 'Sending Link...' : 'Request Reset Link'}
              </button>
            </form>
          )}
        </div>
