        ...
        <div className="flex items-center justify-center py-20">
          <div className="max-w-sm w-full text-center space-y-5 animate-in fade-in zoom-in-95 duration-500">
            ...
            <div className="space-y-1">
              <h1 className="text-2xl font-bold text-foreground font-heading">Acknowledgement Complete!</h1>
            ...
            {/* Summary card */}
            <div className="bg-secondary border border-border rounded-2xl p-5 text-left space-y-3">
              <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Acknowledgement Summary</h3>
              {[
            ...
            <div className="flex gap-3">
              <Link href="/employee" className="flex-1 py-2.5 bg-secondary border border-border hover:bg-secondary rounded-xl text-xs font-bold text-card-foreground transition-colors text-center">
                Go to Dashboard
              </Link>
            </div>
          </div>
      ...
      <EmployeeLayout>
        <div className="flex items-center justify-center py-20">
          <div className="max-w-sm w-full text-center space-y-5 animate-in fade-in zoom-in-95 duration-500">
            ...
            <div className="space-y-1">
              <h1 className="text-2xl font-bold text-foreground font-heading">Acknowledgement Submitted</h1>
              <p className="text-sm text-muted-foreground">
            ...
            <div className="bg-secondary border border-border rounded-2xl p-5 text-left space-y-3">
              <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Submitted Acknowledgement</h3>
              {[
    ...
    <EmployeeLayout>
      <div className="relative overflow-hidden space-y-5">
        ...
        <div className="flex items-center gap-4">
          <Link href="/employee/policies" className="w-8 h-8 bg-secondary border border-border rounded-lg flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors">
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div>
            <h1 className="text-xl font-bold text-foreground font-heading">Final Acknowledgement</h1>
            <p className="text-xs text-muted-foreground">Onboarding Step 4 of 5</p>
          </div>
        </div>
                ...
                <div className="flex flex-col items-center gap-1">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all ${
                    step.active
                      ...
                      : step.n < 4 ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-600'
                      : 'bg-secondary border-border text-muted-foreground'
                  }`}>
                    {step.n < 4 ? <CheckCircle2 className="w-4 h-4" /> : step.n}
                  </div>
                  <span className={`text-[9px] font-semibold ${step.active ? 'text-blue-600' : step.n < 4 ? 'text-emerald-600' : 'text-neutral-600'}`}>
                    {step.label}
                  </span>
                </div>
                {i < 4 && (
        ...
        {total === 0 ? (
          <div className="bg-secondary border border-border rounded-2xl p-5 flex items-start gap-3">
            <AlertCircle className="w-5 h-5 text-muted-foreground shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-bold text-card-foreground">No Policies Assigned</p>
          ...
          <div className="bg-red-500/5 border border-red-500/20 rounded-2xl p-5 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-red-600">Policies Incomplete</p>
              <p className="text-xs text-muted-foreground mt-1">
                You have {pending} pending {pending === 1 ? 'policy' : 'policies'} to accept before you can proceed with the final acknowledgement.
              </p>
              <Link href="/employee/policies" className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-red-600 hover:text-red-300">
          ...
          <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-2xl p-5 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-emerald-600">All Policies Accepted</p>
              <p className="text-xs text-muted-foreground mt-1">
        ...
        <div className="bg-secondary border border-border rounded-2xl overflow-hidden">
          {/* Header */}
          <div className="bg-gradient-to-r from-blue-600/10 to-indigo-600/10 border-b border-border px-6 py-4 flex items-center gap-3">
            <div className="w-9 h-9 bg-blue-500/10 border border-blue-500/20 rounded-xl flex items-center justify-center">
              <Shield className="w-5 h-5 text-blue-600" />
            </div>
            <div>
              <h2 className="text-sm font-bold text-foreground">Official Declaration</h2>
              <p className="text-[10px] text-muted-foreground">Read carefully before signing</p>
            </div>
            <div className="ml-auto text-right">
              <p className="text-[10px] text-muted-foreground">Policies Accepted</p>
              <p className="text-sm font-bold text-foreground">{accepted} / {total}</p>
            </div>
          </div>

          {/* Declaration text */}
          <div className="p-6">
            <div className="bg-card border border-border rounded-xl p-5 text-xs text-card-foreground leading-[1.85] whitespace-pre-wrap font-mono">
              {DECLARATION_TEXT}
            </div>
          </div>
        </div>
        ...
        <div className="bg-secondary border border-border rounded-2xl p-5 space-y-3">
          <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Digital Record (Auto-captured)</h3>
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={item.label} className="flex items-center gap-2.5 bg-card rounded-xl p-3">
        ...
        {/* Signature Form */}
        <form onSubmit={handleSubmit} className="bg-secondary border border-border rounded-2xl p-6 space-y-5">
          <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Your Digital Signature</h3>

          <div>
            <label className="block text-[11px] font-bold text-muted-foreground uppercase tracking-wide mb-1.5">
              Full Name (as per employment records) <span className="text-red-600">*</span>
            </label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-card border border-border rounded-xl pl-10 pr-4 py-2.5 text-sm text-foreground placeholder-neutral-600 focus:outline-none focus:border-blue-500 disabled:opacity-50 transition-colors"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 p-4 bg-card border border-border rounded-xl">
            ...
            <div>
              ...
              <p className="text-xs text-card-foreground font-semibold">{currentDate} · {currentTime}</p>
            </div>
            <p className="ml-auto text-[10px] text-muted-foreground italic">Auto-captured</p>
          </div>

            <label className={`flex items-start gap-3 cursor-pointer group p-4 rounded-xl border transition-all ${checked ? 'bg-blue-500/5 border-blue-500/30' : 'bg-card border-border hover:border-border'} ${(total > 0 && !allAccepted) ? 'opacity-50 pointer-events-none' : ''}`}>
              <div className={`w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 mt-0.5 transition-all ${checked ? 'bg-blue-600 border-blue-600' : 'border-border group-hover:border-blue-500'}`}>
                {checked && <CheckCircle2 className="w-3 h-3 text-foreground fill-white" />}
              </div>
          ...
          {error && (
            <div className="bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3 text-xs text-red-600 font-semibold flex items-center gap-2">
          ...
          <button
            ...
            className={`w-full py-3.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all ${
              (total === 0 || allAccepted) && checked && fullName.trim()
                ? 'bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-foreground shadow-lg shadow-emerald-950/30'
