...
export default function PolicyReaderPage() {
    ...
    return (
      <div className="min-h-screen bg-card flex items-center justify-center p-6">
        <div className="text-center space-y-3">
          ...
          <p className="text-foreground font-semibold">Policy not found or not assigned to you.</p>
          <Link href="/employee/policies" className="text-blue-600 text-sm hover:underline">← Back to Policies</Link>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-card flex items-center justify-center p-6">
        <div className="text-center space-y-4 max-w-sm animate-in fade-in zoom-in-95 duration-300">
          ...
          <h2 className="text-xl font-bold text-foreground">Policy Accepted!</h2>
          <p className="text-sm text-muted-foreground">
            You've accepted <strong className="text-foreground">{policy.title}</strong>.
  ...
  return (
    <div className="min-h-screen bg-card flex flex-col">
      {/* Top bar */}
      <div className="bg-card border-b border-border backdrop-blur-sm sticky top-0 z-20">
        <div className="max-w-4xl mx-auto px-6 py-3 flex items-center gap-4">
          <Link href="/employee/policies" className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground font-semibold transition-colors">
            <ArrowLeft className="w-3.5 h-3.5" /> All Policies
          </Link>
          <div className="h-4 w-px bg-secondary" />
          <p className="text-xs text-muted-foreground truncate flex-1">{policy.title}</p>
          ...
          {policy.accepted && (
            <span className="flex items-center gap-1 text-[10px] font-bold text-emerald-600 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-lg">
      ...
      <div className="flex-1 max-w-4xl mx-auto w-full px-6 py-6 space-y-5">
        ...
        <div className="bg-secondary border border-border rounded-2xl p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-2 flex-wrap mb-2">
                <span className="text-[10px] font-bold text-muted-foreground bg-secondary rounded-lg px-2 py-0.5 flex items-center gap-1">
                  <Hash className="w-3 h-3" /> {policy.policyNumber}
                </span>
                <span className="text-[10px] font-bold text-blue-600 bg-blue-500/10 border border-blue-500/20 rounded-lg px-2 py-0.5 flex items-center gap-1">
                  <Tag className="w-3 h-3" /> {CATEGORY_LABELS[policy.category] || policy.category}
                </span>
                <span className="text-[10px] font-mono text-muted-foreground">Version {policy.version}</span>
              </div>
              <h1 className="text-2xl font-bold text-foreground font-heading">{policy.title}</h1>
              {policy.description && (
          ...
          {policy.effectiveDate && (
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground mt-4">
              ...
              Effective from: <span className="text-card-foreground">{new Date(policy.effectiveDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' })}</span>
            </div>
          )}
        </div>
        ...
        <div
          ref={contentRef}
          className="bg-secondary border border-border rounded-2xl overflow-y-auto"
          style={{ maxHeight: '55vh' }}
        >
          <div className="p-8">
            <div className="text-sm text-card-foreground leading-[1.85] whitespace-pre-wrap font-[Inter]">
              {policy.content}
            </div>

            {/* Fade sentinel — appears at bottom */}
            <div className="mt-8 pt-6 border-t border-border">
              <p className="text-xs text-muted-foreground text-center italic">— End of Policy Document —</p>
            </div>
          </div>
        </div>

        {/* Acceptance panel */}
        <div className={`bg-secondary border rounded-2xl p-5 transition-all duration-300 ${hasScrolledToBottom ? 'border-blue-500/30' : 'border-border opacity-60'}`}>
          {!hasScrolledToBottom && (
          ...
          {hasScrolledToBottom && !policy.accepted && (
            <div className="space-y-4">
              <label className="flex items-start gap-3 cursor-pointer group">
                <div className={`w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 mt-0.5 transition-all ${acknowledged ? 'bg-blue-600 border-blue-600' : 'border-border group-hover:border-blue-500'}`}>
                  {acknowledged && <CheckCircle2 className="w-3 h-3 text-foreground fill-white" />}
                </div>
                <input type="checkbox" className="sr-only" checked={acknowledged} onChange={(e) => setAcknowledged(e.target.checked)} />
                <p className="text-sm text-card-foreground leading-relaxed">
                  I have read and understood this policy — <strong className="text-foreground">{policy.title}</strong>.
          ...
          {policy.accepted && (
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-emerald-600">Policy Accepted</p>
              ...
              <Link href="/employee/policies" className="ml-auto flex items-center gap-1 text-xs text-blue-600 hover:text-blue-300 font-semibold">
