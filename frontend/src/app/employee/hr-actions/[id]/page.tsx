...
const SeverityBadge = ({ severity }: { severity: string }) => {
  ...
  return (
    <span className={`text-xs font-bold uppercase tracking-wider border px-3 py-1 rounded ${styles[severity] || 'bg-secondary text-muted-foreground border-border'}`}>
      {severity}
    </span>
  );
};
...
const StatusBadge = ({ status }: { status: string }) => {
  const styles: Record<string, { bg: string; icon: React.ReactNode }> = {
    DRAFT: { bg: 'bg-background0/10 text-muted-foreground border-neutral-500/20', icon: <FileText className="w-4 h-4" /> },
    ISSUED: { bg: 'bg-blue-500/10 text-blue-600 border-blue-500/20', icon: <AlertCircle className="w-4 h-4" /> },
    VIEWED: { bg: 'bg-purple-500/10 text-purple-600 border-purple-500/20', icon: <Eye className="w-4 h-4" /> },
    SENT: { bg: 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20', icon: <Clock className="w-4 h-4" /> },
    ACKNOWLEDGED: { bg: 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20', icon: <CheckCircle2 className="w-4 h-4" /> },
    RESPONSE_PENDING: { bg: 'bg-amber-500/10 text-amber-600 border-amber-500/20', icon: <Clock className="w-4 h-4" /> },
    ...
    CANCELLED: { bg: 'bg-red-500/10 text-red-600 border-red-500/20', icon: <XCircle className="w-4 h-4" /> },
  };
    ...
    return (
      <EmployeeLayout>
        <div className="flex flex-col items-center justify-center h-96 gap-4">
          ...
          <p className="text-muted-foreground">Failed to load HR action</p>
          <Link
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
        ...
        <div className="bg-card border border-border rounded-2xl p-6">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-4">
              <span className="text-lg text-muted-foreground font-mono font-bold">
                {action.actionNumber}
              </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* Subject & Type */}
            <div className="bg-card border border-border rounded-2xl p-6">
              <h2 className="text-2xl font-bold text-foreground mb-2">{action.subject || '—'}</h2>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-card-foreground whitespace-pre-wrap">{action.reason || '—'}</p>
                </div>
                ...
                {action.correctiveAction && (
                  <div>
                    ...
                    <p className="text-card-foreground whitespace-pre-wrap">{action.correctiveAction}</p>
                  </div>
                )}
                ...
                {action.additionalRemarks && (
                  <div>
                    ...
                    <p className="text-card-foreground whitespace-pre-wrap">{action.additionalRemarks}</p>
                  </div>
                )}
              </div>
            </div>
                ...
                {canRespond ? (
                  <div className="space-y-4">
                    ...
                    <textarea
                      ...
                      className="w-full h-32 bg-secondary border border-border rounded-xl p-4 text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-amber-500/50"
                    />
                    <button
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
                      <p className="text-sm text-emerald-600 font-semibold flex items-center gap-2 mb-2">
                        <CheckCircle2 className="w-4 h-4" />
                        Response Submitted
                      </p>
                      <p className="text-sm text-muted-foreground">
                    ...
                    {action.responseText && (
                      <div>
                        <h4 className="text-sm font-bold text-muted-foreground uppercase mb-2">Your Response:</h4>
                        <p className="text-card-foreground whitespace-pre-wrap">{action.responseText}</p>
                      </div>
                    )}
                  </div>
                ) : (
              ...
              <div className="bg-card border border-green-500/20 rounded-2xl p-6">
                ...
                <div className="space-y-2">
                  ...
                  {action.resolvedRemarks && (
                    <div className="mt-4">
                      <h4 className="text-sm font-bold text-muted-foreground uppercase mb-2">Remarks:</h4>
                      <p className="text-card-foreground whitespace-pre-wrap">{action.resolvedRemarks}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-card border border-red-500/20 rounded-2xl p-6">
                ...
                <div className="space-y-2">
                  ...
                  {action.cancelledReason && (
                    <div className="mt-4">
                      <h4 className="text-sm font-bold text-muted-foreground uppercase mb-2">Reason:</h4>
                      <p className="text-card-foreground whitespace-pre-wrap">{action.cancelledReason}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-6">
            {/* Key Dates */}
            <div className="bg-card border border-border rounded-2xl p-6">
              <h3 className="text-sm font-bold text-muted-foreground uppercase mb-4">Key Dates</h3>
              <div className="space-y-3">
                <div className="flex items-start gap-3">
                  <Calendar className="w-4 h-4 text-muted-foreground mt-0.5" />
                  <div className="text-sm">
                    <p className="text-muted-foreground">Incident Date</p>
                    <p className="text-foreground">{formatDate(action.incidentDate)}</p>
                  </div>
                </div>
                {action.issuedAt && (
                  <div className="flex items-start gap-3">
                    ...
                    <div className="text-sm">
                      ...
                      <p className="text-foreground">{formatDateTime(action.issuedAt)}</p>
                    </div>
                  </div>
                )}
                {action.viewedAt && (
                  <div className="flex items-start gap-3">
                    ...
                    <div className="text-sm">
                      ...
                      <p className="text-foreground">{formatDateTime(action.viewedAt)}</p>
                    </div>
                  </div>
                )}
                {action.acknowledgedAt && (
                  <div className="flex items-start gap-3">
                    ...
                    <div className="text-sm">
                      ...
                      <p className="text-foreground">{formatDateTime(action.acknowledgedAt)}</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            {/* Issued By */}
            <div className="bg-card border border-border rounded-2xl p-6">
              <h3 className="text-sm font-bold text-muted-foreground uppercase mb-4">Issued By</h3>
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm">
                  {action.issuedBy?.employee ? (
                    <>
                      ...
                      <p className="text-muted-foreground">{action.issuedBy.email}</p>
                    </>
                  ) : (
                    <p className="text-muted-foreground">HR Department</p>
                  )}
                </div>
              </div>
            </div>
            ...
            {canAcknowledge && (
              <div className="bg-card border border-border rounded-2xl p-6">
                <h3 className="text-sm font-bold text-muted-foreground uppercase mb-4">Actions</h3>
                <button
                  onClick={() => acknowledgeMutation.mutate()}
                  disabled={acknowledgeMutation.isPending}
                  className="w-full px-6 py-3 bg-blue-500 text-foreground font-bold rounded-lg hover:bg-blue-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
