      ...
      <div className="space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Raise support tickets, flag grievances, and communicate directly with HR operations.</p>
          </div>
          <Link
            href="/employee/complaints/create"
            className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 rounded-xl text-xs font-extrabold text-foreground transition-all shadow-lg shadow-blue-950/40"
          >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Open Tickets" value={stats?.open} icon={MessageSquare} color="bg-blue-500/10 text-blue-600" />
          <StatCard title="Waiting Response" value={stats?.pendingReply} icon={Clock} color="bg-amber-500/10 text-amber-600" />
          <StatCard title="Resolved" value={stats?.resolved} icon={CheckCircle2} color="bg-emerald-500/10 text-emerald-600" />
          <StatCard title="Closed" value={stats?.closed} icon={XCircle} color="bg-secondary text-muted-foreground" />
        </div>
        ...
        <div className="bg-secondary border border-border rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <select
              ...
              className="bg-background border border-border rounded-xl px-4 py-2.5 text-xs text-foreground focus:outline-none focus:border-blue-500 appearance-none text-left bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%23666%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E')] bg-[length:0.65em_auto] bg-[right_1rem_center] bg-no-repeat"
            >
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-secondary">
                  <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-6 py-4">Ticket ID</th>
                  <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-6 py-4">Subject</th>
                  <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-6 py-4">Category</th>
                  <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-6 py-4">Priority</th>
                  <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-6 py-4">Status</th>
                  <th className="text-left text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-6 py-4">Created Date</th>
                  <th className="text-right text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-6 py-4">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-neutral-800/40">
                {isLoading ? (
                  <tr>
                    <td colSpan={7} className="text-center py-20 text-muted-foreground text-xs font-semibold">
                  ...
                  <tr>
                    <td colSpan={7} className="text-center py-20 text-muted-foreground text-xs">
                      <div className="flex flex-col items-center gap-3">
                        <div className="w-12 h-12 bg-secondary rounded-2xl flex items-center justify-center">
                          <LifeBuoy className="w-6 h-6 text-muted-foreground" />
                        </div>
                        <p className="text-muted-foreground">No support tickets found in your list.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  complaintsList.map((item: any) => (
                    <tr key={item.id} className="hover:bg-secondary/50 transition-colors">
                      <td className="px-6 py-4 text-xs font-mono text-muted-foreground font-bold">
                        {item.complaintNumber}
                      </td>
                      <td className="px-6 py-4">
                        <div>
                          <p className="text-xs font-bold text-foreground leading-normal truncate max-w-xs">{item.title}</p>
                          <p className="text-[10px] text-muted-foreground truncate max-w-xs mt-0.5">{item.description}</p>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-xs text-card-foreground">
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link
                          ...
                          className="inline-flex items-center gap-1 text-[10px] font-extrabold text-blue-600 hover:text-blue-300"
                        >
