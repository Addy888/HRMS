  ...
  const loginMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.user?.role !== 'HR') {
        setError('Only registered HR Administrators are permitted to access this portal.');
        return;
      }
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500"
              />
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                ...
                <Link href="/forgot-password" className="text-xs text-blue-400 hover:text-blue-300 font-medium">Forgot Password?</Link>
              </div>
              <div className="relative">
                <input
                  ...
                  className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-2.5 pr-12 text-sm text-white focus:outline-none focus:border-blue-500"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-neutral-500 hover:text-neutral-350"
                >
            ...
            <div className="flex items-center gap-2 py-1">
              ...
              />
              <label htmlFor="remember" className="text-xs text-neutral-400 font-medium select-none cursor-pointer">Remember me for 30 days</label>
            </div>
