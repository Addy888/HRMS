          ...
          {subItems.map((item) => (
            <Link
              ...
              className={`flex items-center px-4 py-2 rounded-lg text-sm transition-all ${
                pathname === item.href
                  ? 'bg-secondary/80 text-card-foreground border-l-2 border-blue-500'
                  : 'text-muted-foreground hover:text-card-foreground hover:bg-secondary/30'
              }`}
            >
              {item.label}
            </Link>
      ...
      <div className="flex min-h-screen bg-background text-foreground antialiased">
        ...
        <aside className="hidden md:flex flex-col w-64 border-r border-border bg-card p-6 space-y-6 shadow-sm">
          ...
          <div className="bg-secondary/50 border border-border rounded-xl p-4 flex flex-col gap-1 shadow-sm">
            ...
            <span className="text-sm font-semibold truncate text-card-foreground">{user?.email || 'hr@fcs.com'}</span>
            <span className={`text-[10px] ${isHRAdmin ? 'bg-purple-500/10 text-purple-600 border-purple-500/20' : 'bg-blue-500/10 text-blue-600 border-blue-500/20'} border px-2 py-0.5 rounded w-max mt-1 font-bold`}>
              {roleBadge}
            </span>
          </div>
            ...
            <div className="md:hidden fixed inset-0 z-50 flex">
              ...
              <aside className="relative flex flex-col w-64 bg-card p-6 border-r border-border animate-in slide-in-from-left duration-200 shadow-xl">
                <div className="flex items-center justify-between mb-6">
                  <span className="font-heading text-lg font-bold text-card-foreground">HR Menu</span>
