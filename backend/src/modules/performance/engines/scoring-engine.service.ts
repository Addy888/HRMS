...
export class ScoringEngineService {
  ...
  calculateKPIRating(
    ...
    if (thresholds) {
      if (thresholds.excellent && achievementPercentage >= thresholds.excellent)
        return 5;
      if (thresholds.good && achievementPercentage >= thresholds.good) return 4;
      if (
        thresholds.satisfactory &&
        achievementPercentage >= thresholds.satisfactory
      )
        return 3;
  ...
  calculateCompetencyScore(
    competencies: Omit<CompetencyScore, 'average'>,
  ): number {
    const ratings = [
  ...
  calculate360FeedbackScore(
    ...
    const typeGroups = feedbacks.reduce(
      (acc, fb) => {
  ...
  private aggregateCompetencies(
  ...
  ): CompetencyScore & { average: number } {
    ...
    const allValues = Object.values(aggregate).filter(
      (v) => typeof v === 'number',
    );
    aggregate.average =
      allValues.length > 0
        ? allValues.reduce((sum: number, v: any) => sum + v, 0) /
          allValues.length
        : 0;
