...
export class UploadsController {
  ...
  @ApiParam({ name: 'filename', description: 'Complaint attachment filename' })
  ...
  @ApiParam({ name: 'filename', description: 'Company policy filename' })
  ...
  @ApiParam({ name: 'filename', description: 'Attendance filename' })
  ...
  private async serveFile(
    ...
    console.log(`   CONFIGURED UPLOAD_DIR: ${process.env.UPLOAD_DIR || './uploads (default)'}`);
    console.log(`   PROCESS CWD: ${process.cwd()}`);
    ...
    // If file doesn't exist, list what's actually in the folder
    if (!existsSync(filePath)) {
      ...
      if (existsSync(folderPath)) {
        ...
        try {
          const files = readdirSync(folderPath);
          if (files.length === 0) {
            console.log(`      ⚠️  Folder is empty - no files uploaded yet`);
          } else {
            console.log(`      Found ${files.length} file(s):`);
            files.slice(0, 20).forEach((f, idx) => {
              const fPath = join(folderPath, f);
              const fStats = statSync(fPath);
              console.log(`      ${idx + 1}. ${f} (${fStats.size} bytes, modified: ${fStats.mtime.toISOString()})`);
            });
            if (files.length > 20) {
        ...
        console.log(`   💡 TIP: Run the server to auto-create upload folders`);
      }
      
      console.log('\n   🔍 TROUBLESHOOTING:');
      console.log(`      1. Check if file exists in database: SELECT * FROM Document WHERE fileUrl LIKE '%${filename}%';`);
      console.log(`      2. If database has record but file missing: File was deleted or moved`);
      console.log(`      3. If database has no record but file exists: Run restore-documents.js`);
      console.log(`      4. If neither exists: Document was never uploaded or was fully deleted`);
      
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      throw new NotFoundException(`File not found: ${filename}. Check server logs for details.`);
    }
